let tmdbURL = "https://api.themoviedb.org/4/list/";
let beeMoviesFans = [
    { "userName": "William", "listID": 7093377, "api_key": "e46fb5199dc632c90b3ae4b76af3cbf4" },
    { "userName": "Eric", "listID": 7093269, "api_key": "e46fb5199dc632c90b3ae4b76af3cbf4" },
    { "userName": "David", "listID": 7093379, "api_key": "e46fb5199dc632c90b3ae4b76af3cbf4" },
];

let urls = [];
for (fan in beeMoviesFans) {
    urls.
        push(`${tmdbURL}${beeMoviesFans[fan].listID}?api_key=${beeMoviesFans[fan].api_key}&page=1&language=fr`);
}

const imageBaseURL = 'https://image.tmdb.org/t/p/';
let imageWidth = 'w500';
let poster_path = '/uAaXfaJpKjmsA3F1KgowcRJPKPM.jpg';

async function fetchTmdbBeeMovies() {
    let tmdbJsonLists = await Promise.all(
        urls.map(url => fetch(url)
            .then(response => response.json()))
    );
    return tmdbJsonLists;
}

async function createMoviesWannaBeesJsonDatabase() {
    let moviesWannaBees = [];
    await fetchTmdbBeeMovies().then((tmdbJsonLists) => {
        for (user in tmdbJsonLists) {
            moviesWannaBees.push(...tmdbJsonLists[user].results);
        }
    })
...
async function loadSiteContent() {
    ...
    console.log(moviesWannaBees);
    let beesSliderCardsHTML='';
        ...
        for (movie in moviesWannaBees){
            beesSliderCardsHTML += `       
                <div class="home__card">
                        ...
                        <div>
                            <h2>${moviesWannaBees[movie].title}</h2>
                            <ul>
                                <li>Free</li>
                                <li>Action</li>
                                <li>${moviesWannaBees[movie].release_date}</li>
                            </ul>
                        </div>
            ...
            beesCatalogueCardsHTML += `
                <div  class="col-6 col-sm-4 col-lg-3 col-xl-2">
                    <div class="card">
                        ...
                        <h3 class="card__title"><a href="details.html">${moviesWannaBees[movie].title}</a></h3>
                        <ul class="card__list">
                            <li>Free</li>
                            <li>Comedy</li>
                            <li>${moviesWannaBees[movie].release_date}</li>
